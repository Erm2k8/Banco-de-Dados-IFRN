...
# ===============================================
# VIEWS CRIADAS PARA O EXERCÍCIO DE VIEWS NO BD
# ===============================================

def view_ufs(request):
    """Exibe lista de UFs distintas da tabela Município"""
    template = "view_ufs.html"
    
    try:
        conn = obter_conexao()
        cursor = conn.cursor()
        
        sql = "SELECT * FROM view_ufs ORDER BY uf"
        data = cursor.execute(sql).fetchall()
...
def view_cursos_populares(request):
    """Exibe cursos com >= 2000 ofertas"""
    template = "view_cursos_populares.html"
    
    try:
        conn = obter_conexao()
        cursor = conn.cursor()
        
        sql = "SELECT * FROM view_cursos_populares ORDER BY quantidade_ofertas DESC"
        data = cursor.execute(sql).fetchall()
...
def view_enade_por_area_curso(request):
    """Exibe média do ENADE por área e curso (média >= 3)"""
    template = "view_enade_por_area_curso.html"
    
    try:
        conn = obter_conexao()
        cursor = conn.cursor()
        
        sql = "SELECT * FROM view_enade_por_area_curso ORDER BY area, curso"
        data = cursor.execute(sql).fetchall()
